...
process mapping_reads {
	conda '/home/miniconda3/envs/myenv/'
	maxForks 15
	...
	script:
	"""
	dragen-os -r /home/pipelines/NextSeq_mutation_detector_leukemia/dragen_reference/ -1 ${paired_read1} -2 ${paired_read2} --RGID AML --RGSM ${Sample} > ${Sample}.sam
	sed -i 's/PL:PL0/PL:ILLUMINA/g' ${Sample}.sam
	#bwa mem -R "@RG\\tID:AML\\tPL:ILLUMINA\\tLB:LIB-MIPS\\tSM:${Sample}\\tPI:200" -t 20 ${params.genome} ${paired_read1} ${paired_read2} > ${Sample}.sam
	"""
} 

process sam_conversion{
	maxForks 15
	publishDir "$PWD/Final_Output/${Sample}/", mode: 'copy', pattern: '*.fxd_sorted.bam'
	publishDir "$PWD/Final_Output/${Sample}/", mode: 'copy', pattern: '*.fxd_sorted.bam.bai'
	publishDir "${PWD}/${Sample}/Annovar_Modified/", mode: 'copy'
	input:
...
workflow MIPS {
	...
	main:
	trimming_trimmomatic(samples_ch) | mapping_reads | sam_conversion | mark_duplicates
	//| pair_assembly_pear | mapping_reads | sam_conversion
